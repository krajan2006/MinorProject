...
public class AnalogClockApp extends JFrame 
{
    ...
    private class ClockPanel extends JPanel
    {
        ...
        protected void paintComponent(Graphics g)
        {
            ...
            // Calculate the radius of the clock, leaving a margin of 20 pixels
            centerX = getWidth() / 2; 
            // Calculate the X-coordinate of the clock center
            centerY = getHeight() / 2; 
            // Calculate the Y-coordinate of the clock center
            
            // Draw clock face (circle)
            g2d.setColor(Color.black); 
            // Set the color for the clock face
            ...
            g2d.setColor(Color.GREEN); // Set the color for the hour numbers
            
            for(int hour = 1; hour <= 12; hour++)
            {
                double angle = Math.toRadians(90-(360/12) * hour); 
                // Calculate the angle for placing the hour number
                int x = (int)(centerX + clockRadius * 0.8 * Math.cos(angle)); 
                // Calculate the X-coordinate of the hour number position
                int y = (int)(centerY - clockRadius * 0.8 * Math.sin(angle)); 
                // Calculate the Y-coordinate of the hour number position
                g2d.drawString(Integer.toString(hour), x - 7, y + 5); 
                // Draw the hour number with an offset
            }
            
            // Get current time
            SimpleDateFormat sdf = new SimpleDateFormat("HH:mm:ss"); 
            // Create a date formatter for hours, minutes, and seconds
            ...
            // Calculate the angle for the minutes hand
            double secondsAngle = Math.toRadians(90 - (360 / 60) * seconds); 
            // Calculate the angle for the seconds hand
            
            // Draw clock hands
            // Draw the hour hand in yellow
            drawClockHand(g2d, centerX, centerY, clockRadius * 0.5, hoursAngle, 6, Color.green);
            // Draw the minute hand in yellow
            drawClockHand(g2d, centerX, centerY, clockRadius * 0.7, minutesAngle, 4, Color.green);
            // Draw the second hand in red
            drawClockHand(g2d, centerX, centerY, clockRadius * 0.8, secondsAngle, 2, Color.green);
